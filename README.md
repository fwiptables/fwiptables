...
###  fwiptables install.                     
    ...
    put bit TYPE: chmod 755 ./fwiptables-version-24-06-29-bash.sh     
    install TYPE: ./fwiptables-version-24-06-29-bash.sh install       
...
###  [ version ] [ Show version ] [ version md ]            
   [ info ] [ Somes details ]                 
   [ info ] [ Basename program   ] fwiptables-version-24-06-29-bash.sh               
   [ info ] [ Realpath program   ] /root/f-iptables-version/fwiptables-version-24-06-29-bash.sh               
   [ info ] [ Number Version     ] 24-06-29                
   ...
   /root/.config/fwiptables/fwiptables-preferences/default-preferences-24-06-29            
   [ info ] [ File  Format       ]    
   /root/f-iptables-version/fwiptables-version-24-06-29-bash.sh: Bourne-Again shell script, Unicode text, UTF-8 text executable       
...
###        [ with optional output ] [ Example Description ]
   ...
   | depends                  | fwiptables-version-24-06-29-bash.sh depends
   | List firewall saved      | fwiptables-version-24-06-29-bash.sh names
   | List configs saved       | fwiptables-version-24-06-29-bash.sh all-custom
   | Get info speed ipv4      | fwiptables-version-24-06-29-bash.sh speed-ip4
   | Show listen sockets      | fwiptables-version-24-06-29-bash.sh sockets
   | List last options        | fwiptables-version-24-06-29-bash.sh autolog
   | modify default variables | fwiptables-version-24-06-29-bash.sh preferences-modify
  
###  [ Several examples with optional output ]
  
         [ with optional output ] [ Example Description ]
  
   | Example with info        | fwiptables-version-24-06-29-bash.sh txt info             
   | Example with expert      | fwiptables-version-24-06-29-bash.sh txt expert           
   | Example with code ip4    | fwiptables-version-24-06-29-bash.sh cli code ip4         
   | Example with list rules  | fwiptables-version-24-06-29-bash.sh cli lsn4             
   | Example with nodes       | fwiptables-version-24-06-29-bash.sh gui nodes            
   | Example with web browser | fwiptables-version-24-06-29-bash.sh gui web kernel.org   
  
   | Launch client web firewall in silent   | fwiptables-version-24-06-29-bash.sh silent client-web
   | List iptables rules with output txt    | fwiptables-version-24-06-29-bash.sh txt ls4
   | List firewall with output cli whiptail | fwiptables-version-24-06-29-bash.sh cli-wiptail names
   | List sockets ip with output gui yad    | fwiptables-version-24-06-29-bash.sh gui-yad sockets
   | All options in text menu               | fwiptables-version-24-06-29-bash.sh cli-menu-dialog
   | All options in window menu             | fwiptables-version-24-06-29-bash.sh gui-menu-yad
   | All options in window roll             | fwiptables-version-24-06-29-bash.sh gui-roll-zenity
   | All options in window shell            | fwiptables-version-24-06-29-bash.sh gui-shell-yad
  
### 
### 
###  [ options ] [ List general options for fwiptables ] [ list-options md ]
  
###  fwiptables-version-24-06-29-bash.sh [optional-output] first_option [second_option]  
###   optional-output                                                     
...
###   firewall-wallsystem                                                 
   ...
   server-mumble server-gateway server-sql server-asterisk server-proxy  
   server-samba                                                          
###   options-easy                                                        
