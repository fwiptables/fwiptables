...
####
#### para valor vacio
if [ "$1" == "$NULL" ]; then echo  "### launch: $0 [ -null ] directory|probe|status|start|stop|restart|mount|umount  [ imagen-hd ] [ imagen.iso ] "; exit ; fi
...
#### variables estaticas
if [ "$second_option"  == "$NULL" ] ; then qemu_vm="$directory_images/debian-i386.qcow2" ; else qemu_vm=$second_option ; fi
...
####
##### directory
if [ "$first_option" == "directory" ]
then echo " [ ok ] [ directory ] [ $directory_images ]"
exit; fi
##### whith probe
