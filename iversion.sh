...
cmd_version="cmd-11-06"          # its version
...
if [ "$listado_proxy" -eq "0" ];
then echo "$text_md [ info ] Without proxy in export variables"
else $command_sudo -u root bash -c export | -i grep "_PROXY" ; fi
echo
...
####
####
var0="$(echo $title_md $cmd_shortdescription from $cmd_realpath version $cmd_version $title_md )"
...
var9="$(echo client_port_tcp=http,https,http-alt,ssh )"
...
var33="$(echo allow_string_denied=no )"
eng33="$(echo $title_md void to if drop string, or no to no )"
spa33="$(echo $title_md vacio para denegar cadena, o no para no )"
var34="$(echo allow_string_allowed=no )"
eng34="$(echo $title_md void to if allow string, or no to no )"
spa34="$(echo $title_md vacio para permitir cadena, o no para no )"
...
echo "$text_md"
echo "$text_md  Uninstall fwiptables. TYPE:   $file_installed uninstall     "
echo "$text_md"
$cmd_realpath ver
...
--width=$graphicall_width --height=$graphicall_height --entry \
--title="[new-mini-config]" \
--entry-text="Input file name to new mini configuration")" ;
...
####
####
$allow_shield_maxtries $allow_use_legacy  \
$allow_use_ipv4  $command_iptables_legacy -A INPUT \
-p tcp -m multiport --dports ssh -m state --state NEW \
-m recent --set --name shield-tries \
-m comment --comment "shield tries" &> /dev/null
$allow_shield_maxtries $allow_use_legacy  $allow_use_ipv4  \
...
####
####
$allow_shield_maxtries $allow_use_nft  $allow_use_ipv4 \
...
####
####
$allow_shield_maxtries $allow_use_legacy  $allow_use_ipv6 \
...
####
####
$allow_shield_maxtries $allow_use_nft  $allow_use_ipv6 \
$command_ip6tables_nft -A INPUT \
-p tcp -m multiport --dports $config_shield_port \
...
$command_iptables_legacy -A INPUT \
-p tcp -m multiport --dports "$config_shield_port" \
-m state --state NEW -m recent --set --name shield-tries \
-m comment --comment "shield tries" &> /dev/null
$allow_shield_maxtries $allow_use_legacy $allow_use_ipv4 \
$command_iptables_legacy -A INPUT \
-m recent --name shield-tries --update --seconds 3600 \
...
#### -m comment --comment "logserver udp" &> /dev/null
$allow_use_legacy  $allow_use_ipv6   $command_ip6tables_legacy \
-A INPUT -p tcp -m multiport --sports $server_port_tcp -j LOG \
-m comment --comment "logserver tcp" &> /dev/null
####
####
#### $allow_use_legacy  $allow_use_ipv6   $command_ip6tables_legacy -A OUTPUT \
