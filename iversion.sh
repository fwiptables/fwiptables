...
cmd_version="12-05"                          # number version
...
echo "$title_md File generated by fwiptables expert-conf-clientproxy user:pass@ip:port"
...
cat $theproxy
else "$title_md [ info ] [ if it wants conf proxy and then launch: $cmd_basename expert-conf-clientproxy ]"
fi
exit ; fi
####
####
...
echo "$text_md"
echo "$text_md With rules: ipv4, ipv6, ebtables, arptables.    "
echo "$text_md With netfilter neftables, netfilter xtables.    "
echo "$text_md With tools ip, wizards ready to gen rules.      "
echo "$text_md Permit save and load rules with custom files.   "
echo "$text_md Permit shield to ssh or other servers choosed,  "
echo "$text_md Permit string algoritmo, limit bandwidth,       "
echo "$text_md Permit host whitelist, host blacklist,          "
